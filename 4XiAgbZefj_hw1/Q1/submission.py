...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        """
        tup = (source, target)
        tup_inv = (target, source)
        print('test')
        print(self.nodes)
        if (source != target) and (tup not in self.edges):
            print('edge not found in edges')
    ...
    def max_degree_nodes(self) -> dict:
        ...
        # traverse each edge in edges
        # have a running list to store multiples of nodes that tie
        # save in dict where key = node_id and value = integer
        max_nodes = {}
        for edge in self.edges:
            # print(edge)
            for i in edge:  # each element in tuple
                if i in max_nodes:  # check if tuple element is in the dict
                    max_nodes[i] += 1
...
class TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id: str, limit: int = None, exclude_ids: list = None) -> list:
        ...
        Important: the exclude_ids processing should occur prior to limiting output. 3/movie/{movie_id}/credits?api_key=<<api_key>>&language=en-US
        """

        conn = http.client.HTTPSConnection('api.themoviedb.org')
        conn.request("GET",
                     "/3/movie/{0}/credits?api_key=6709a2302552720d38d328517e2ab82d&language=en-US".format(movie_id,
                                                                                                           self.api_key))
    ...
    def get_movie_credits_for_person(self, person_id: str, vote_avg_threshold: float = None) -> list:
        ...
        conn = http.client.HTTPSConnection('api.themoviedb.org')
        conn.request("GET",
                     "/3/person/{0}/credits?api_key=6709a2302552720d38d328517e2ab82d&language=en-US".format(person_id,
                                                                                                           self.api_key))
        response = conn.getresponse()
        movie_credit_lst = []
        if response.status == 200:
            ...
            movies = decoded['cast']
            # : run through cast
            # : 2. select only credits where vote_avg >= vote_avg_threshold (if not none)
            # : 3. Grab the corresponding dicts and add to list
            # : 4. return movie_credit_lst
            #for movie_credit in movies:
            for credit in movies:
                if vote_avg_threshold is not None and credit['vote_average'] >= vote_avg_threshold:
                    movie_credit_lst.append(credit)
            return movie_credit_lst
        else:
...
# END LOOP
#
#todo: Your graph should not have any duplicate edges or nodes
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key='6709a2302552720d38d328517e2ab82d')

    movie_credits = tmdb_api_utils.get_movie_credits_for_person(person_id='2975', vote_avg_threshold=8.0)

    cast = tmdb_api_utils.get_movie_cast(movie_id='100', limit=3, exclude_ids=[973, 974])
    #print(cast)
    ...
    for movie in movie_credits:
         ...
         for member in cast:
            ...
            graph.add_edge(source=str(movie[0]), target=member['name'])
